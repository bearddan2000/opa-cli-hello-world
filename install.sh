...
function start-up(){
    ...
    git clone https://github.com/rzuckerm/opa-docker-image.git docker-image

    cp -R $PWD/bin $PWD/docker-image

    cd $PWD/docker-image

    sudo docker build -t $docker_img_name .

    sudo docker run -ti --rm -p 80:8080 -w /code -v $PWD/bin:/code $docker_img_name bash ./run.sh
